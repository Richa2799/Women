...
public class CandidateController {
	 ...
	 @Autowired
	    ...
	    @GetMapping("/getCandidatesByEmail/{email}")
	    public Optional<Candidate> getCandidateByEmail(@PathVariable(value = "email") String email)
	        throws ResourceNotFoundException {
	    	Optional<Candidate> c = candidateService.getByEmail(email);
	        return c;
	    }
	    
	    @GetMapping("/candidatesByEmail/{email}")
	    public boolean getAdminByEmail(@PathVariable(value = "email") String email)
	        throws ResourceNotFoundException {
	    	boolean val = candidateService.findByEmail(email);
	        return val;
	    }
	    @CrossOrigin(origins = "http://localhost:4200")
